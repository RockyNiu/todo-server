...
var initialize = _.once(function() {
	...
	// I must ensure uniqueness accross the two properties because two users can
	// have the same token (ex: in apn, 1 token === 1 device)
...
var add = function(user, deviceType, token) {
	...
	pushItem.save(function(err) {
		var meta = 'save pushItem:[user ' + user + '] [device: ' + deviceType
				+ '] [' + token + ']\n';
		errorLogfile.write(meta);
		if (err) {
			errorLogfile.write('[ERROR:] '+ meta);
		} else {
			accessLogfile.write('[DONE:] '+ meta);
		}
	});
};
...
var updateTokens = function(fromToArray) {
	fromToArray.forEach(function(tokenUpdate) {
		PushAssociation.findOneAndUpdate({
...
var getForUsers = function(users, callback) {
	...
	// PushAssociation.where('user').in(users).exec(wrappedCallback);
};

var removeForUser = function(user) {
	PushAssociation.remove({
		user : user
	}, function(err) {
		if (err)
			console.dir(err);
